...
bool Shape::isOverlapping(const Shape &s) const {
    if (getRightX()<s.getLeftX() || s.getRightX()<getLeftX()) {
        return false;
    }
    else if (getBottomY()<s.getTopY() || s.getBottomY()<getTopY()) {
        return false;
    }
    else {
        return true;
